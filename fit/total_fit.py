...
best, covar = curve_fit(f.quadratic, a*a, E_mean/a,
                        p0=(1., -1/24, 3/640), sigma=error_E/a)
plt.plot(x, f.quadratic(x*x, *best), ls='-', c='r')
...
chi = f.red_chi_sq(
    E_mean/a, f.quadratic(a*a, *best), error_E/a, dof=d)
...
best, covar = curve_fit(f.quadratic, a*a, mel,
                        p0=(1/np.sqrt(2), -1*np.sqrt(2)/32, 5*np.sqrt(2)/1024), sigma=error_mel)
plt.plot(x, f.quadratic(x*x, *best), ls='-', c='r')

d = len(a*a) - len(best)  # Degree of freedom of the fit

plt.xlabel('a')
plt.ylabel('Matrix element')
plt.grid(linestyle=':')

chi = f.red_chi_sq(mel, f.quadratic(a*a, *best), error_mel, dof=d)
